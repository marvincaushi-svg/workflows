"""Declarative control plane for WorkflowOS automations.
        ...
        automations.append(
            {
                ...
                "owner": _require_string(automation.get("owner"), f"{field}.owner"),
                "objective": objective,
                "output": output,
...
def _idempotency_key(automation_id: str, case_id: str, event_ref: str) -> str:
    payload = {
...
def build_event_execution_plan(
    ...
    *,
    ...
    completed_pairs = {
        (item.get("automation_id"), item.get("case_id"))
    ...
    for event_index, raw_event in enumerate(_require_list(events, "events")):
        ...
        for automation in matches:
            key = _idempotency_key(automation["id"], case_id, event_ref)
            item = {
            ...
            missing_dependencies = [
                ...
                if (dependency, case_id) not in completed_pairs
            ]
...
def build_schedule_execution_plan(
    ...
    *,
    ...
    for automation in catalog["automations"]:
        ...
        slot = (
            f"{automation['id']}:{local.date().isoformat()}:{trigger['local_time']}"
        )
        base = {
            "automation_id": automation["id"],
            "schedule_slot": slot,
